...
class EmailService:
    ...
    def __init__(self, sender_email: str, sender_password: str, smtp_server: str = 'smtp.gmail.com', smtp_port: int = 587, user_id: Optional[str] = None):
    ...
    def _get_resume_path(self, template_type: str) -> Optional[str]:
        """Get the appropriate resume path based on template type and user"""
        if self.user_id:
            # Check user-specific resumes directory first
            user_resume_dir = PROJECT_ROOT / 'users' / self.user_id / 'resumes'
            resume_path = user_resume_dir / f"{template_type}_resume.pdf"
            if resume_path.exists():
                logger.info(f"Using user custom resume: {resume_path}")
                return str(resume_path)
            
            # Look for any pdf in user resumes directory as fallback
            if user_resume_dir.exists():
                pdfs = list(user_resume_dir.glob("*.pdf"))
                if pdfs:
                    logger.warning(f"Category resume not found. Falling back to first available user resume: {pdfs[0]}")
                    return str(pdfs[0])

        # Global fallbacks
        global_resume_dir = PROJECT_ROOT / 'resumes'
        resume_path = global_resume_dir / f"{template_type}_resume.pdf"
        if resume_path.exists():
            return str(resume_path)
            
        # Fallback to software_dev_resume.pdf globally
        fallback_path = global_resume_dir / 'software_dev_resume.pdf'
        if fallback_path.exists():
            return str(fallback_path)

        logger.warning(f"No resume found for template type: {template_type}")
        return None
